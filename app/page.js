    ...
    <div className="min-h-screen">
      ...
      <section className="relative overflow-hidden min-h-[600px]">
        <div className="absolute inset-0">
          ...
          <div className="absolute inset-0" style={{background: 'linear-gradient(to right, rgba(123,30,44,0.95) 0%, rgba(123,30,44,0.75) 50%, rgba(123,30,44,0.2) 100%)'}} />
        </div>
        <div className="relative container mx-auto px-4 py-24 md:py-36 grid md:grid-cols-2">
          <div className="text-white">
            ...
            <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="font-display text-5xl md:text-7xl font-bold leading-tight mb-6">
              Wangian <span className="text-caisy-gold italic">Mewah</span>,<br />Harga Terjangkau
            </motion.h1>
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-lg text-white/80 mb-8 max-w-lg">
              Koleksi parfum dupe eksklusif terinspirasi dari brand dunia seperti Chanel, Dior, YSL. Wangi tahan lama dengan formula premium.
            </motion.p>
          ...
          <motion.div animate={{ y: [0, -20, 0] }} transition={{ duration: 4, repeat: Infinity }} className="hidden md:flex justify-end items-center">
            <div className="w-80 h-96 rounded-2xl bg-gradient-to-br from-caisy-gold/30 to-white/10 backdrop-blur border border-caisy-gold/30 flex items-center justify-center">
              <span className="font-display text-7xl text-caisy-gold">C</span>
            </div>
          </motion.div>
        </div>
      </section>
      ...
      <section className="bg-white border-b border-caisy-gold/20">
        <div className="container mx-auto px-4 py-8 grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { icon: ShieldCheck, title: '100% Original', desc: 'Formula premium' },
