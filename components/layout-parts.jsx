        ...
        <div className="flex items-center gap-2">
          ...
          {user ? (
            <div className="relative">
              ...
              {userMenuOpen && (
                <div className="absolute right-0 top-12 bg-white border border-caisy-primary/30 rounded-lg shadow-xl py-2 min-w-[200px]"
      ...
      <div className="container mx-auto px-4 py-12 grid md:grid-cols-4 gap-8">
        ...
        <div>
          ...
          <ul className="space-y-2 text-sm text-white/80">
            ...
            <li>
              <a href={`https://wa.me/${cfg.whatsapp_cs}`} target="_blank" rel="noreferrer" className="hover:text-white transition">
                WhatsApp CS
              </a>
            </li>
          </ul>
        ...
        <div>
          ...
          <a href={`mailto:${cfg.email_cs}`} className="text-sm text-white/80 hover:text-white block">{cfg.email_cs}</a>
          <a href={`https://wa.me/${cfg.whatsapp_cs}`} target="_blank" rel="noreferrer" className="text-sm text-white/80 hover:text-white block mt-1">
            {cfg.phone}
          </a>
          <div className="flex gap-3 mt-3">
            {cfg.instagram && (
              <a href={cfg.instagram} target="_blank" rel="noreferrer" aria-label="Instagram"
                className="w-9 h-9 rounded-full bg-white/10 hover:bg-white/30 flex items-center justify-center transition text-xs font-bold">IG</a>
            )}
            {cfg.tiktok && (
              <a href={cfg.tiktok} target="_blank" rel="noreferrer" aria-label="TikTok"
                className="w-9 h-9 rounded-full bg-white/10 hover:bg-white/30 flex items-center justify-center transition text-xs font-bold">TT</a>
            )}
            {cfg.facebook && (
              <a href={cfg.facebook} target="_blank" rel="noreferrer" aria-label="Facebook"
                className="w-9 h-9 rounded-full bg-white/10 hover:bg-white/30 flex items-center justify-center transition text-xs font-bold">FB</a>
            )}
          </div>
        </div>
      </div>
      <div className="border-t border-white/10 text-center text-xs py-4 text-white/60">
        © {new Date().getFullYear()} {cfg.brand_name} {cfg.brand_tagline}. All rights reserved.
      </div>
    </footer>
  )
}
...
export function WhatsAppButton() {
  const cfg = useSiteConfig()
  if (!cfg.whatsapp_cs) return null

  return (
    <a href={`https://wa.me/${cfg.whatsapp_cs}`} target="_blank" rel="noreferrer"
